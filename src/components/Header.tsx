                ...
                <div className="container mx-auto px-6 py-5 flex justify-between items-center">
                    <div className="flex items-center gap-4 md:gap-0">
                        <button className="md:hidden" onClick={() => setIsMenuOpen(true)}>
                            <svg className={`w-7 h-7 transition-colors duration-300 ${scrolled || isHovered ? 'text-gray-900' : 'text-white'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7"></path></svg>
                        </button>
                        <div className="w-20 h-20 flex-shrink-0">
                            <img src={kdcLogo} alt="Kids Design Company" className="w-full h-full object-contain" />
                        </div>
                    </div>
                    <nav className="hidden md:flex items-center gap-8 transition-colors duration-300">
                        <Link to="/" className={`font-medium hover:text-[#0080A0] transition-colors ${scrolled || isHovered ? 'text-gray-900' : 'text-white'}`}>Our Services</Link>
                        <Link to="/industries" className={`font-medium hover:text-[#0080A0] transition-colors ${scrolled || isHovered ? 'text-gray-900' : 'text-white'}`}>Industries we work with</Link>
                        <Link to="/consultation" className={`font-medium hover:text-[#0080A0] transition-colors ${scrolled || isHovered ? 'text-gray-900' : 'text-white'}`}>Consultation</Link>
                        <Link to="/blogs" className={`font-medium hover:text-[#0080A0] transition-colors ${scrolled || isHovered ? 'text-gray-900' : 'text-white'}`}>Blogs</Link>
                        <Link to="/plans" className={`font-medium hover:text-[#0080A0] transition-colors ${scrolled || isHovered ? 'text-gray-900' : 'text-white'}`}>Packages</Link>
                    </nav>
                    <a href="#" className={`font-semibold py-3 px-8 rounded-full text-sm transition-all duration-300 shadow-md hover:shadow-lg ${
                        scrolled || isHovered
